#!/bin/bash
# Emotional Speech SFT Training for CosyVoice2-0.5B LLM
# Dataset: ESD + RAVDESS (filtered, no CREMA-D)
# Training: LLM-only supervised fine-tuning
...
echo "========================================="
echo "Training CosyVoice2-0.5B LLM for Emotional Speech"
echo "========================================="
...
echo "Pretrained checkpoint: $pretrained_model_dir/llm.pt"
...
if [ ! -f "$pretrained_model_dir/llm.pt" ]; then
    echo "ERROR: Pretrained LLM model not found at $pretrained_model_dir/llm.pt"
...
# Train LLM only (not flow/hifigan)
